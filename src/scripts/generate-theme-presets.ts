 ...
 * Default theme colors are fetched from /styles/styles.css.
 ...
 * - This generation step is automated using a Husky pre-commit hook.
...
const globalStylesPath = path.resolve(__dirname, "../styles/styles.css");

let globalContent = "";
try {
  globalContent = fs.readFileSync(globalStylesPath, "utf8");
} catch (error) {
  console.error(`❌ Could not read styles.css at ${globalStylesPath}`);
  console.error(error);
  process.exit(1);
}
...
try {
  ...
  console.error("❌ Unexpected error while generating theme presets:", error);
  process.exit(1);
}
