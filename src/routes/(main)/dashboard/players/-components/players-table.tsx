...
export function PlayersTable({ table }: { table: ReactTable<DataTableFeatures, PlayerRow> }) {
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  key={row.id}
                  className="h-9 border-border/60 transition-colors hover:bg-muted/35"
      ...
      <div className="flex items-center justify-between px-4">
        <div className="flex items-center gap-4 text-muted-foreground text-sm">
          <div className="flex items-center gap-2">
            ...
            >
              <SelectTrigger size="sm" className="w-20" id="players-rows-per-page">
